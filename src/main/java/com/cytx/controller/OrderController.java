...
import com.cytx.utils.PaymentUtil;
...
public class OrderController {
    ...
    public String submitOrder(HttpServletRequest request, Order order, Double price, Model model){

        order.setOrderTotalPrice(price*order.getOrderAmount());
        ...
        if (isValid) {
            try {
                ...
                if (r9_BType.equals("1")) {
                    ...
                    orderService.updateStatusToPayById(order.getOrderId());
                    System.out.println("付款的订单编号是："+r6_Order);

                    // 浏览器重定向（展示给客户端的界面）
                    response.sendRedirect("http://localhost:8080/cytx/paySuccessPage?no="+r6_Order+"&code="+order.getCode());
